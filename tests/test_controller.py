...
class RuleExtractionTests(unittest.TestCase):
    def test_extracts_one_host(self):
        self.assertEqual(extract_hosts('Host(`app.home.prettybaked.com`)'), ['app.home.prettybaked.com'])
    ...
    def test_normalizes_and_allowlists(self):
        ...
        services = [{'Spec': {'Name': 'app', 'Labels': {
            'unifi-dns.enable': 'true',
            'traefik.http.routers.app.rule': 'Host(`APP.HOME.PRETTYBAKED.COM.`)'}}}]
        self.assertEqual(desired_records(services, ['home.prettybaked.com']), {'app.home.prettybaked.com': 'docker-swarm'})

    def test_target_override_duplicate_and_conflict(self):
        svc = lambda name, target: {'Spec': {'Name': name, 'Labels': {
            'unifi-dns.enable': 'true', 'unifi-dns.target': target,
            'traefik.http.routers.r.rule': 'Host(`same.home.prettybaked.com`)'}}}
        desired, conflicts = desired_records([svc('a', 'one'), svc('b', 'two')], ['home.prettybaked.com'], with_conflicts=True)
...
class ReconcileTests(unittest.TestCase):
    ...
    def test_preserves_unowned_records_and_api_failure(self):
        api = FakeUnifi([{'key': 'manual.home.prettybaked.com', 'value': 'manual.local'}], fail_create=True)
        controller = Controller(api, ['home.prettybaked.com'], {})
        with self.assertRaises(RuntimeError):
            controller.reconcile([service('new', 'Host(`new.home.prettybaked.com`)', 'docker-swarm')])
        controller.reconcile([])
    ...
    def test_service_update_and_removal_converge(self):
        ...
        controller.reconcile([])
        self.assertEqual(api.created, [('old.home.prettybaked.com', 'docker-swarm.local'), ('new.home.prettybaked.com', 'edge.local')])
        self.assertEqual(api.deleted, ['old.home.prettybaked.com', 'new.home.prettybaked.com'])


def service(name, rule, target):
    return {'Spec': {'Name': name, 'Labels': {'unifi-dns.enable': 'true', 'unifi-dns.target': target, 'traefik.http.routers.app.rule': rule}}}
...
class FakeUnifi:
    ...
    def create(self, host, target):
        if self.fail_create: raise RuntimeError('API unavailable')
        self.created.append((host, target)); self.records.append({'key': host, 'value': target})
    def update(self, host, target): self.updated.append((host, target))
    def delete(self, host): self.deleted.append(host)
