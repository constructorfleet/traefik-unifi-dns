...
def desired_records(services, zones, default_target='docker-swarm', localdomain='local', with_conflicts=False):
...
class Controller:
    ...
    def reconcile(self, services):
        desired, self.conflicts = desired_records(services, self.zones, self.default_target, self.localdomain, True)
        ...
        for host, target in desired.items():
            ...
            if record is None:
                ...
                LOG.info(json.dumps({'hostname': host, 'target': fq_target, 'action': 'create', 'result': 'ok'}))
            elif self.ownership.get(host) and record.get('value') != fq_target:
                self.unifi.update(host, fq_target)
                self.ownership[host] = fq_target
                LOG.info(json.dumps({'hostname': host, 'target': fq_target, 'action': 'update', 'result': 'ok'}))
